...
In traditional finance, a Level 4 trading account in a brokerage firm may be able to sell naked puts and naked calls by only posting about 5x less collateral than in a Level 1 account.
...
Users could also choose to deposit a single type of collateral: Bob could decide to deposit 4500 Dai, and Charlie could decide to deposit 2.5ETH and they would all have the same buying power (although they would all depend on the current price differently).

This amount of collateral will be used to back the purchase and sale of options.
...
However, in all those scenarios, any change in price or accumulation of long premium could make Alice's account insolvent and lead her account to be [liquidated](docs/panoptic-protocol/liquidations).
...
It is set at a fixed 10% for pool utilization less than 50% and decrease to 5% at 90% or more.
...
The collateralization ratio for selling an option is set at a fixed 20% for pool utilization less than 50% and increases to 100% at pool utilization >90%.
